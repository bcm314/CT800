...
#ifdef PC_PRINTF
#include <stdio.h>
extern void Play_Show_Board(void);
...
DATA_SECTION static int signed_material_table[PIECEMAX] =
...
{
    Util_Memzero(T_T,     (MAX_TT + CLUSTER_SIZE) * sizeof(TT_ST));
    Util_Memzero(Opp_T_T, (MAX_TT + CLUSTER_SIZE) * sizeof(TT_ST));
}
...
void Hash_Cut_Tables(unsigned int clear_counter)
{
    ...
    for (i = 0; i < MAX_TT+CLUSTER_SIZE; i++)
    {
        ...
        {
            ttentry->cmove = MV_NO_MOVE_CMASK;
            ttentry->pos_hash_upper_h = 0;
            ttentry->pos_hash_upper_l = 0;
            ttentry->value = 0;
            ttentry->flag = NO_FLAG;
            ttentry->depth = 0;
        }
...
void Hash_Init_Stack(void)
{
    ...
    Hash_Clear_Tables();
    Util_Memzero(P_T_T, sizeof(TT_PTT_ST) * (PMAX_TT+1));
    Util_Memzero(P_T_T_Rooks, sizeof(TT_PTT_ROOK_ST) * (PMAX_TT+1));
}
...
/*returns whether a position is checkmate. needed for the corner case with the
  ...
  This routine is called from Hash_Check_For_Draw() which is called from
  pre-search and Search_Negascout() and not from Quiescence. Since
  Hash_Is_Checkmate() cannot be inlined, its stack usage does not add up to
  every Negascout recursion level even if Hash_Check_For_Draw() should get
  inlined in LTO.
  */
static int NEVER_INLINE Hash_Is_Checkmate(void)
{
            ...
            if (LIKELY((ttentry->flag & 0xFCU) == additional_bits))
            {
                if (ttentry->cmove)
                {
                    ...
                    if (UNLIKELY(Mvgen_Check_Move_Legality(tt_move, colour) == 0))
                    {
                        ...
                        Play_Show_Board();
                        if (colour == WHITE)
                            fprintf(stderr, "invalid white PV move: %s\r\n", Play_Translate_Moves(tt_move));
                        else
                            fprintf(stderr, "invalid black PV move: %s\r\n", Play_Translate_Moves(tt_move));

                        if (Starting_Mv & 1)
                            fprintf(stderr, "%3d. ...  ", Starting_Mv/2 + 1);

                        for (int k = Starting_Mv + 1; k <= mv_stack_p; k++)
                        {
                            if (k & 1)
                                fprintf(stderr, "%3d. ", k/2 + 1);

                            fprintf(stderr, "%s ", Play_Translate_Moves(move_stack[k].move));
            ...
            if (LIKELY((ttentry->flag & 0xFCU) == additional_bits))
            {
                if (ttentry->cmove)
                {
                    ...
                    if (UNLIKELY(Mvgen_Check_Move_Legality(tt_move, colour) == 0))
                    {
                        ...
                        Play_Show_Board();
                        if (colour == WHITE)
                            fprintf(stderr, "invalid white move: %s\r\n", Play_Translate_Moves(tt_move));
                        else
                            fprintf(stderr, "invalid black move: %s\r\n", Play_Translate_Moves(tt_move));
